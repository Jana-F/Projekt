...
def profile(screen_name):
    ...
    if not user.get('do_check'):
        return render_template('no_data.html', screen_name=user['screen_name'])

    til = date.today()
    since = til - timedelta(days=7)

    graph = display_user_data(user, since, til)
    return render_template('profile.html', graph=graph, screen_name=user['screen_name'])
...
def track(screen_name):
    ...
    return redirect(url_for('profile', screen_name=user['screen_name']))
